...
export default function VendorProfile() {
  ...
  const fetchProfile = async () => {
    ...
    try {
      ...
      setUserData({ name, email, companyName, role });
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
          <div className="flex items-center gap-6 mb-8">
            <div className="w-24 h-24 bg-emerald-600 rounded-3xl flex items-center justify-center text-white text-3xl font-black shadow-lg">
              {userData.name.substring(0, 2).toUpperCase()}
            </div>
            <div>
              <h3 className="text-2xl font-black text-slate-900 mb-1">{userData.name}</h3>
              <p className="text-sm text-slate-500 font-medium">{userData.companyName || "No company name"}</p>
              <span className="inline-block mt-2 px-3 py-1 bg-emerald-50 text-emerald-600 rounded-lg text-xs font-black uppercase">
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-start gap-4 p-4 bg-slate-50 rounded-2xl">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm font-black text-slate-900">{userData.companyName || "Not set"}</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-4 p-4 bg-slate-50 rounded-2xl">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm font-black text-slate-900 capitalize">{userData.role || "Vendor"}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm">
            ...
            <div className="space-y-3">
              <button
                ...
                className="w-full py-3 px-4 bg-emerald-600 text-white rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-emerald-700 transition-all"
              >
                Edit Profile
              </button>
              <button
