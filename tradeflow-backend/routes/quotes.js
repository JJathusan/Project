...
import { verifyToken } from '../middleware/auth.js'; // MUST IMPORT THIS

const router = express.Router();

// 1. Create Quote Request (Buyer or Admin)
router.post('/request', verifyToken, verifyBuyerOrAdmin, async (req, res) => {
  try {
...
router.get('/vendor', verifyToken, verifyVendorOrAdmin, async (req, res) => {
  try {
...
router.get('/buyer', verifyToken, verifyBuyerOrAdmin, async (req, res) => {
  try {
...
router.patch('/:id/respond', verifyToken, verifyVendorOrAdmin, async (req, res) => {
  try {
...
// 5. Accept Quote (Buyer or Admin)
router.post('/:id/accept', verifyToken, verifyBuyerOrAdmin, async (req, res) => {
  try {
